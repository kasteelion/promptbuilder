...
def main():
    ...
    parser.add_argument("--name", type=str, help="Specifically target one character by name")
    ...
    if args.name:
        missing_queue = [q for q in missing_queue if q['name'].lower() == args.name.lower()]
        if not missing_queue:
            print(f"Character '{args.name}' not found or does not meet criteria.")
            return
