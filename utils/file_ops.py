...
def atomic_write(filepath: Path, content: str, encoding: str = 'utf-8', create_backup: bool = True) -> bool:
    ...
    try:
        ...
        temp_fd, temp_path = tempfile.mkstemp(dir=str(filepath.parent), prefix=f'.{filepath.name}.', suffix='.tmp')
        try:
...
def create_backup(filepath: Path, max_backups: int = 5) -> Optional[Path]:
    ...
    try:
        ...
        # Prune old backups
        existing = sorted(backup_dir.glob(f'{filepath.stem}.*.bak'), key=lambda p: p.stat().st_mtime)
