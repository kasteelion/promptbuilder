...
class TestColorSchemesTeams(unittest.TestCase):
    def setUp(self):
        with open("data/color_schemes.md", "r", encoding="utf-8") as f:
            self.content = f.read()
            
    def test_utils_parse_color_schemes(self):
        schemes = parse_color_schemes("data/color_schemes.md")
        self.assertIn("UCLA", schemes)
        self.assertIn("team", schemes["UCLA"])
        self.assertEqual(schemes["UCLA"]["team"], "The Coastal Bears")
