...
def main():
    ...
    #     return
    print("\n[Step 2] Skipping Connectivity Check (script missing)...")

    # 3. Quality & Integrity Audit (Included Descriptiveness)
    if not run_command(f"python auditing/quality_audit.py", "Step 3: Quality & Descriptiveness Audit"):
        return

    # 3.5 Outfit Precision Audit
    if not run_command(f"python auditing/audit_outfit_quality.py", "Step 3.5: Outfit Precision ('Scientific') Audit"):
        return
    ...
    # 5. Sankey Diagram
    if not run_command(f"python auditing/generate_sankey_diagram.py", "Step 5: Generate Distribution Visualization (Sankey)"):
        return

    # 5.5 Vibe Cohesion Report
    if not run_command(f"python auditing/vibe_cohesion_report.py", "Step 5.5: Analyze Vibe Cohesion"):
        return

    # 5.6 Tag Distribution Flow
    if not run_command(f"python auditing/generate_tag_sankey.py", "Step 5.6: Generate Tag Distribution Flow"):
        return
    
    # 6. Best/Worst Analysis
    if not run_command(f"python auditing/find_best_worst.py", "Step 6: Analyze Best & Worst Prompts"):
        return
