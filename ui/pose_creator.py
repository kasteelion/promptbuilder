...
• Overall energy/mood of the pose"""
        ...
        # Pose name
        ttk.Label(main_frame, text="Pose Name:", font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 4))
        ...
        # Description
        ttk.Label(main_frame, text="Pose Description:", font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 4))
        ttk.Label(main_frame, text="Describe the body position, gestures, and expression", foreground="gray", font=("Segoe UI", 8)).pack(anchor="w")
...
Example: Standing confidently with feet shoulder-width apart, arms crossed over chest, slight smile with direct eye contact, relaxed but assertive energy."""
    ...
    def _create_pose(self):
        ...
        if not name:
            messagebox.showerror("Validation Error", "Please enter a pose name.", parent=self.dialog)
            return
        
        if not description or description.startswith("[Body position]"):
            messagebox.showerror("Validation Error", "Please enter a pose description.", parent=self.dialog)
            return
