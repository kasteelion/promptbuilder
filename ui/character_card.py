...
class CharacterCard(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        add_btn = ttk.Button(btn_frame, text="➕ Add", command=self._on_add_clicked, width=7)
        add_btn.pack(side="left", expand=True, fill="x", padx=(0, 2))

        # Edit button
        edit_btn = ttk.Button(btn_frame, text="✏️ Edit", command=self._on_edit_clicked, width=7)
    ...
    def _display_photo(self, photo_path):
        ...
        try:
            ...
            self.photo_canvas.create_image(x, y, image=self.photo_image, anchor="nw")
