...
class StateManager:
    ...
    def undo(self):
        ...
        if not self.undo_manager.can_undo():
            ...
            notify(self.root, "Undo", "Nothing to undo", level="info", duration=2000)
            return
    ...
    def redo(self):
        ...
        if not self.undo_manager.can_redo():
            # Prefer toast if available, then status bar, then modal
            notify(self.root, "Redo", "Nothing to redo", level="info", duration=2000)
            return
        ...
        try:
            ...
            try:
                notify(
                    self.root,
                    "Success",
                    f"Preset saved: {Path(filepath).name}",
                    level="success",
                    duration=3000,
                )
            except Exception:
                logger.exception("Notification fallback failed for preset save")
        ...
        try:
            ...
            try:
                notify(
                    self.root,
                    "Success",
                    f"Preset loaded: {Path(filepath).name}",
                    level="success",
                    duration=3000,
                )
            except Exception:
                logger.exception("Notification fallback failed for preset load")
    ...
    def load_preset_by_name(self, preset_name: str) -> bool:
        ...
        if not preset_path.exists():
            messagebox.showerror("Not Found", f"Preset '{preset_name}' no longer exists.")
        ...
        try:
            ...
            self.preset_manager.load_preset(str(preset_path), self.restore_state_callback)
    ...
    def export_config(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.exception("Notification fallback failed for config export")
            ...
            messagebox.showerror(
                "Export Error", "Could not export configuration; see log for details"
            )
            return False
    ...
    def import_config(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.exception("Notification fallback failed for config import")
            ...
            messagebox.showerror(
                "Import Error", "Could not import configuration; see log for details"
            )
            return False
