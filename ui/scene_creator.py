...
• Environmental details"""
        ...
        # Scene name
        ttk.Label(main_frame, text="Scene Name:", font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 4))
        self.name_var = tk.StringVar()
        name_entry = ttk.Entry(main_frame, textvariable=self.name_var, font=("Segoe UI", 10))
        name_entry.pack(fill="x", pady=(0, 10))
        
        # Description
        ttk.Label(main_frame, text="Scene Description:", font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 4))
        ttk.Label(main_frame, text="Describe the setting, lighting, and atmosphere", foreground="gray", font=("Segoe UI", 8)).pack(anchor="w")
...
Example: Cozy coffee shop interior, warm ambient lighting, wooden tables, comfortable seating, steaming cups on counter."""
    ...
    def _load_categories(self):
        ...
        if not categories:
            categories = ["Indoor Settings", "Outdoor Settings", "Urban & Industrial", "Fantasy", "Other"]
...
Example: Cozy coffee shop interior, warm ambient lighting, wooden tables, comfortable seating, steaming cups on counter."""
        ...
        if not name:
            messagebox.showerror("Validation Error", "Please enter a scene name.", parent=self.dialog)
            return
